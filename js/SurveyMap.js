...
if(type === "avJourneyTime"){
  ...
  // var week = f.options[f.selectedIndex].value;
        ...
        self_item.stations.forEach(function(d,i){
          ...
          heatMapData.push({location: new google.maps.LatLng(d.position.lat, d.position.lon), weight: val[0].Entry_Week.toString()});
...
//
//         var markers = heatMapData.map(function(location, i) {
...
self_item.heatmap.setOptions({radius: 15, maxIntensity: 10000, gradient: ['#010107', '#02020C', '#030312', '#050417', '#07051D', '#0A0723', '#0D0829', '#100A2F', '#140B35', '#170C3B', '#1B0C41',
 ...
 '#C43C4E', '#C83F4C', '#CB4149', '#CF4446', '#D24644', '#D54941', '#D84C3E', '#DB4F3B', '#DE5338', '#E15635', '#E45A32', '#E65D2F', '#E9612B', '#EB6528', '#ED6925', '#EF6D22', '#F1711E', '#F3751B', '#F47A18', '#F67E14',
  ...
  d3.csv("tubedata/RODS_2016/Entry/Distance_travelled/By_station.csv", function(error, csvData){
    ...
    var kms = menu.options[menu.selectedIndex].value;
...
// for(var counter in val[0]){
//   console.log(counter);
//
// }

  //Construct object with all the information for heatmap based on what the user selects
  citymap.push({title: d.title, center: {lat: d.position.lat, lng: d.position.lon}, kilo: val[0]["" + kms + ""] });

}


              for (var city in citymap) {
                    heatMapData.push({location: new google.maps.LatLng(citymap[city].center.lat, citymap[city].center.lng), weight: citymap[city].kilo});
                  }
...
self_item.heatmap.setMap(map);
self_item.heatmap.setOptions({radius: 15, maxIntensity: 1, gradient: [
  'rgba(0, 255, 255, 0)',
  'rgba(0, 255, 255, 1)',
  'rgba(0, 191, 255, 1)',
  'rgba(0, 127, 255, 1)',
...
self_item.markers.push(self_item.heatmap);
  ...
  //from Totals.csv
    ...
    d3.csv("tubedata/RODS_2016/Entry/Distance_travelled/Totals.csv", function(error, csvData){
      ...
      var menu = document.getElementById("categoryMenu");
      var stat = menu.options[menu.selectedIndex].value;
  ...
  if(val[0] != null){
    ...
    //Construct object with all the information for heatmap based on what the user selects
    citymap.push({title: d.title, center: {lat: d.position.lat, lng: d.position.lon}, dist: val[0]["" + stat + ""] });
                ...
                for (var city in citymap) {
                      ...
                      heatMapData.push({location: new google.maps.LatLng(citymap[city].center.lat, citymap[city].center.lng), weight: citymap[city].dist});
                    }
  ...
  self_item.heatmap.setMap(map);
  self_item.heatmap.setOptions({radius: 15, maxIntensity: d3.max(heatMapData).weight, gradient: ['#010107', '#02020C', '#030312', '#050417', '#07051D', '#0A0723', '#0D0829', '#100A2F', '#140B35', '#170C3B', '#1B0C41',
   '#1F0C48', '#230C4E', '#280B53', '#2C0B58', '#310A5D', '#350960', '#3A0963', '#3E0966', '#430A68', '#470B6A', '#4B0C6B', '#4F0D6C', '#540F6D', '#58106E', '#5C126E', '#60136E', '#64156E', '#68166E', '#6C186E', '#70196E',
   '#741B6E', '#781C6D', '#7D1E6D', '#811F6C', '#85216B', '#89226A', '#8D2369', '#912568', '#952667', '#992865', '#9D2964', '#A12B62', '#A52D60', '#A92E5E', '#AD305C', '#B1325A', '#B53458', '#B93656', '#BD3853', '#C03A51',
   '#C43C4E', '#C83F4C', '#CB4149', '#CF4446', '#D24644', '#D54941', '#D84C3E', '#DB4F3B', '#DE5338', '#E15635', '#E45A32', '#E65D2F', '#E9612B', '#EB6528', '#ED6925', '#EF6D22', '#F1711E', '#F3751B', '#F47A18', '#F67E14',
    '#F78311', '#F8870E', '#F98C0A', '#FA9008', '#FB9506', '#FB9A06', '#FC9F07', '#FCA409', '#FCA80D', '#FCAD12', '#FCB217', '#FBB71C', '#FBBC22', '#FAC128', '#F9C72E', '#F8CC35', '#F7D13C', '#F6D643', '#F5DB4B', '#F4E054',
  ...
  d3.csv("tubedata/RODS_2016/Entry/Access mode/Access.csv", function(error, csvData){
    ...
    var menu1 = document.getElementById("categoryMenu");
    var cat = menu1.options[menu1.selectedIndex].value;

    var menu2 = document.getElementById("dayMenu");
    var day = menu2.options[menu2.selectedIndex].value;
...
if(val[0] != null){
  ...
  //Construct object with all the information for heatmap based on what the user selects
  citymap.push({title: d.title, center: {lat: d.position.lat, lng: d.position.lon}, num: val[0]["" + cat + ""] });
              ...
              for (var city in citymap) {
                    ...
                    heatMapData.push({location: new google.maps.LatLng(citymap[city].center.lat, citymap[city].center.lng), weight: citymap[city].num});
                  }
